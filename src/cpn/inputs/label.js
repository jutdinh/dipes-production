...
export default (props) => {
    ...
    function renderFiles(current) {
        ...
        if (Array.isArray(current)) {
            return current.map((file, index) => {
                ...
                if (functions.isExcelDocumentFormat(file)) {
                    ...
                    return (
                        <div className="image-container" key={index} onClick={() => handleDownload(file)} title={`${file.split('/')[2]}`}>
                            <img
        ...
        // Nếu `current` là một chuỗi và không phải là giá trị placeholder như "FILE-0001"
            ...
            return <img src={current} alt="file" className="selected-image-add pointer" style={{ maxWidth: "100%", height: "auto" }} />;
        }
                    ...
                    <div class="form-group">
                        ...
                        <p>
                            {field.DATATYPE === "DATE" || field.DATATYPE === "DATETIME"
                                ...
                                : field.DATATYPE === "BOOL"
                                    ? defaultValue ? field.DEFAULT_TRUE : field.DEFAULT_FALSE
                                    : field.DATATYPE === "FILE"
                                        ? renderFiles(current)
                                        : current}
                        </p>
