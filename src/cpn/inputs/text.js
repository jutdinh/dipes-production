...
const TEXT = (props) => {
        ...
        if (foreignData.length == 0) {
          ...
          fetch(`${proxy()}/api/foreign/data`, {
            ...
            .then((res) => {
              ...
              // console.log(82, res)
              setForeignData([
                ...data.filter((record) => record !== undefined),
              ]);
              setFields(fields);
              const { ref_field_id } = key;
              // console.log(key)
  ...
  const loadOptions = async (search, loadedOptions, { page }) => {
    ...
    const dataWithoutNull = returnedData.filter(
    ...
    if (defaultValue) {
      const foundOption = options.find(
        (option) => option.value === JSON.stringify(defaultValue)
      );
      if (foundOption) {
          ...
          <div class="col-md-6">
            <form>
              <div class="form-group">
                <label for="name">
                  {field.field_name}
                  {!field.NULL && <span style={{ color: "red" }}> *</span>}
                </label>
                {/* <input
