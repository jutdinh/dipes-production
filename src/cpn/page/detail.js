...
export default function COMPONENT() {
  ...
  const { proxy, pages, lang, socket, functions } = useSelector(
    (state) => state
  );
  ...
  const result = pages
    ?.filter(
      (item) => item.type !== "apiview" && item.components?.[0]?.api_detail
    )
    .find((item) => {
      // Lấy id từ api_get
  ...
  const configDetail = functions.findDetailApi(page);
        ...
        .then((res) => {
          ...
          if (success) {
            ...
            const keyFields = serializeParams.map((par) => {
              const field = apiFields.filter(
                (f) => f.fomular_alias == par.fomular_alias
              )[0];
              return field;
            });
            ...
            fetch(`${proxy()}/d/${id_str}/${rawParams}`, {
              headers: {
                Authorization: _token,
              },
            })
                  ...
                  <h5>
                    <label class="pointer" onClick={() => goToHomePage()}>
                      ...
                      <i
                        class={` ${
                          functions
                            .findPropsNameDetailByUrl(page, id_str)
                            ...
                            <div key={field.field_id}>
                              ...
                              field.DATATYPE == "FILE" ? (
                                <Label
                                  ...
                                  field={field}
                                  defaultValue={
                                    initialData[field.fomular_alias]
                                  }
                                />
                              ) : null}
                            </div>
