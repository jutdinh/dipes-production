...
const COMPONENT = () => {
  ...
  useEffect(() => {
    if (
      !dataStatis ||
      (dataStatis.length === 0 && activeTab === "nav-profile_s2")
    ) {
  ...
  const yourComponentArray =
    data_cpn.data[1]?.children.length > 0
      ? data_cpn.data[1]?.children[0]?.component
      : data_cpn.data[1]?.component;
  ////console.log(pages)
    ...
    for (let batch of batches) {
      ...
      try {
        const response = await fetch(
          `${proxy()}${page.components?.[0]?.api_import}`,
          {
            method: "POST",
            headers: {
              Authorization: _token,
  ...
  const dataCheck = dataPrivileges?.find(
    (item) => item.table_id === dataTable_id
  );
  ...
  const redirectToInput = (url_button) => {
    ...
    openTab(
      `/page/${url}/apis/api/${url_button.replace("/ui/", "")}/input_info`
    );
  };
  ...
  const handleDelete = (data, dataUrl) => {
    ...
    if (typeof data === "object" || data !== undefined) {
      const extractedValues = functions.extractValuesFromData(
        functions.findComponentWithDeleteApiUrl(page, dataUrl),
        data
      );
  ...
  const exportToCSV = (csvData, fields) => {
    ...
    if (csvData === "xlsx") {
      ...
      XLSX.writeFile(
        wb,
        `TEMPLATE_EXCEL_${result.toUpperCase()}_${new Date().getTime()}.xlsx`
      );
    } else if (csvData === "csv") {
      const utf8BOM = "\uFEFF";
      ...
      link.setAttribute(
        "download",
        `TEMPLATE_CSV_${result.toUpperCase()}_${new Date().getTime()}.csv`
      );
  ...
  const callApiDataModalExport_PK = (
    data,
    dataUrl,
                          ...
                          <thead>
                            {selectedFields.map((field) => {
                              ...
                              return (
                                <th key={field}>
                                  {header
                                    ? header.field_name || header.display_name
                                    : field}
                                </th>
                              );
                            })}
                        ...
                        <table class="table table-striped excel-preview">
                          <thead class="sticky-header">
                            {selectedFields.map((field) => {
                              ...
                              return (
                                <th key={field} className="header-cell">
                                  {header
                                    ? header.field_name || header.display_name
                                    : field}
                                </th>
