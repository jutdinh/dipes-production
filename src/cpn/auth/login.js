...
export default () => {
  ...
  useEffect(() => {
    ...
    //console.log(_token)
    if (_token !== null && !hasCheckedToken) {
      // Kiểm tra token và biến trạng thái
        ...
        if (success) {
          ...
          fetch(`${proxy()}/auth/activation/check`, {
            ...
            .then((resp) => {
              ...
              if (activated) {
                socket.emit("/dipe-production-user-login", {
                  username: auth.username,
                });
                if (
                  credential.data?.role === "ad" ||
                  credential.data?.role === "uad"
                ) {
                    ...
                    <div className="field">
                      <div class="row">
                        ...
                        <div class="col-md-8" style={{ height: "25px" }}>
                          {authError && (
                            <span class="error-message error-login">
                              {authError}
                            </span>
                          )}
                          {!authError && lable && myParam !== null && (
                            <span
                              style={{
                                fontSize: "14px",
                                fontFamily: "sans-serif",
                              }}
                              class="error-message error-login"
                            >
                              {lang["expired"]}
                            </span>
                          )}
