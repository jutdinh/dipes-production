...
const NavbarItem = ({
  ...
  const toggleMenu = () => {
    if (!isChild) {
      // Chỉ áp dụng cho cấp 1 (top-level)
      ...
      {hasChildren ? (
        <>
          <a
            ...
            className={`dropdown-toggle  ${isChild ? "child-link" : ""}`}
            style={{ paddingLeft }}
          >
          ...
          <ul
            className={`collapse list-unstyled ${isExpanded ? "show " : ""} ${
              isChild ? "child-list" : ""
            }`}
            id={`page_${item.page_id}`}
          >
...
const Navbar = ({ data, isLogged }) => {
  ...
  // if (!data?.is_public && isLogged) {
  //   return null;
  // }

  const handleIsPublicPageOrLogged = (page, children) => {
    if (page.is_public || isLogged) {
      return children;
    }
    return null;
  };
