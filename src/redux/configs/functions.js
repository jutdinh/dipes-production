...
function findPageById(data, pageId) {
  console.log("findPageById", data, pageId);
  for (const item of data) {
    if (item.page_id === pageId) {
      return item; // Tìm thấy ở cấp hiện tại
    }
...
function findGetApi(data) {
  ...
  if (data && data.component && Array.isArray(data.component)) {
    ...
    return tableComponent && tableComponent.props && tableComponent.props.source
      ? tableComponent.props.source.get.url
      : "";
  }
  return null;
}
...
function findPostApi(data) {
  ...
  if (data && data.component && Array.isArray(data.component)) {
    ...
    return tableComponent &&
      ...
      tableComponent.props.buttons
      ? tableComponent.props.buttons.add.api.url
      : "";
  }
...
function findPutApi(data) {
  ...
  if (data && data.component && Array.isArray(data.component)) {
    ...
    return tableComponent &&
      tableComponent.props &&
      tableComponent.props.buttons
      ? tableComponent.props.buttons.update.api
      : "";
  }
  return null;
}
...
function findDeleteApi(data) {
  ...
  if (data && data.component && Array.isArray(data.component)) {
    ...
    return tableComponent &&
      tableComponent.props &&
      tableComponent.props.buttons
      ? tableComponent.props.buttons.delete.api
      : "";
  }
  return null;
}
  ...
  if (data && data.component && Array.isArray(data.component)) {
    for (let i = 0; i < data.component.length; i++) {
      ...
      if (
        ...
        component.props.buttons.delete &&
        component.props.buttons.delete.api
      ) {
...
function findDetailApi(data) {
  ...
  if (data && data.component && Array.isArray(data.component)) {
    ...
    return tableComponent &&
      tableComponent.props &&
      tableComponent.props.buttons
      ? tableComponent.props.buttons.detail.api.url
      : "";
  }
  return null;
}
...
function findExportApi(data) {
  ...
  if (data && data.component && Array.isArray(data.component)) {
    ...
    return tableComponent &&
      tableComponent.props &&
      tableComponent.props.buttons
      ? tableComponent.props.buttons.export.api.url
      : "";
  }
  return null;
}
...
function findSearchApi(data) {
  ...
  if (data && data.component && Array.isArray(data.component)) {
    const tableComponent = data.component.find((comp) => comp.name === "table");
    return tableComponent && tableComponent.props && tableComponent.props.source
      ? tableComponent.props.source.search.url
      : "";
  }
  return null;
}
...
const findPropsNameAddByUrl = (data, targetUrl) => {
  ...
  for (const component of data.component) {
    ...
    if (
      component.props &&
      component.props.buttons &&
      component.props.buttons.add &&
      component.props.buttons.add.api &&
      component.props.buttons.add.api.url.split("/")[2] === targetUrl
    ) {
...
const findPropsNameUpdateByUrl = (data, targetUrl) => {
  ...
  for (const component of data.component) {
    ...
    if (
      component.props &&
      component.props.buttons &&
      component.props.buttons.update &&
      component.props.buttons.update.api &&
      component.props.buttons.update.api.url.split("/")[2] === targetUrl
    ) {
      // Trả về giá trị của props.name
      return component.props.name;
    }
...
const findPropsNameDetailByUrl = (data, targetUrl) => {
  ...
  for (const component of data.component) {
    ...
    if (
      component.props &&
      component.props.buttons &&
      component.props.buttons.detail &&
      component.props.buttons.detail.api &&
      component.props.buttons.detail.api.url.split("/")[2] === targetUrl
    ) {
      // Trả về giá trị của props.name
      return component.props.name;
    }
...
const findPropsNameImportByUrl = (data, targetUrl) => {
  ...
  for (const component of data.component) {
    ...
    if (
      component.props &&
      component.props.buttons &&
      component.props.buttons.import &&
      component.props.buttons.import.api &&
      component.props.buttons.import.api.url.split("/")[2] === targetUrl
    ) {
      // Trả về giá trị của props.name
      return component.props.name;
    }
...
function getComponentText(data) {
  ...
  if (data && data.component && Array.isArray(data.component)) {
    const tableComponent = data.component.find((comp) => comp.name === "table");
    return tableComponent && tableComponent.props && tableComponent.props
      ? tableComponent.props
      : "";
  }
  return null;
...
function detectBrowser(userAgent) {
  ...
  const isChrome =
    !!userAgent.chrome &&
    (!!userAgent.chrome.webstore || !!userAgent.chrome.runtime);

  const isSafari =
    /constructor/i.test(userAgent.HTMLElement) ||
    (function (p) {
      return p.toString() === "[object SafariRemoteNotification]";
    })(
      !userAgent["safari"] ||
        (typeof userAgent.safari !== "undefined" &&
          userAgent.safari.pushNotification)
    );

  const isOpera =
    (!!userAgent.opr && !!userAgent.opr.addons) ||
    !!userAgent.opera ||
    (userAgent.userAgent && userAgent.userAgent.indexOf(" OPR/") >= 0); // Kiểm tra userAgent tồn tại trước khi thực hiện indexOf
  ...
  const isEdgeChromium =
    isChrome &&
    userAgent.userAgent &&
    userAgent.userAgent.indexOf("Edg") !== -1; // Kiểm tra userAgent tồn tại trước khi thực hiện indexOf
...
const dateGenerator = (dateString) => {
  ...
  return `${formatDateNumber(date.getDate())}/${formatDateNumber(
    date.getMonth() + 1
  )}/${date.getFullYear()} lúc ${formatDateNumber(
    date.getHours()
...
const showApiResponseMessage = (status) => {
  ...
  const icon =
    message?.[langItem]?.type === "Thành công" ||
    message?.[langItem]?.type === "Success"
      ? "success"
      : "error";
...
function shortenFileName(fileName) {
  ...
  const prefix =
    namePart.length > maxPrefixLength
      ? namePart.substring(namePart.length - maxPrefixLength)
      : namePart; // Lấy 3 ký tự cuối của phần tên, hoặc toàn bộ nếu ngắn hơn
    ...
    case "TEXT":
      return (
        <input
          ...
          value={value}
          onChange={(e) =>
            handleInputChange(e, field.fomular_alias, e.target.value, search)
          }
...
export default {
  ...
  formatDateCase,
  formatDateMessage,
  translateDateToVietnamese,
  translateDateTimeToVietnamese,
  isEmpty,
  isImageFormat,
  isVideoFormat,
  removeFileExtension,
  resizeImage,
  getAllParamsAfterPageId,
  resizeImageToFit,
  ...
  findExportApi,
  findDetailApi,
  findComponentWithDeleteApiUrl,
  findPropsNameAddByUrl,
  findPropsNameUpdateByUrl,
  findPropsNameImportByUrl,
  findPropsNameDetailByUrl,
  renderInput,
  isExcelDocumentFormat,
  isPdfDocumentFormat,
  isDocDocumentFormat,
  isZipDocumentFormat,
  getFileType,
  isExcelDocumentFormatTableView,
  isPdfDocumentFormatTableView,
  isDocDocumentFormatTableView,
  isZipDocumentFormatTableView,
  isOtherDocumentFormatTableView,
  isImageDocumentFormatTableView,
};
