...
class Api extends Controller {
    ...
    generateRandomData = async (req, res) => {
        ...
        if (verified) {
            const { foreign_table, foreign_value, table, onField, indexField, amount, pattern } = req.body;
            ...
            if (targetTables.length > 0) {
                ...
                const fields = await this.#__fields.findAll({ id: { $in: [onField, indexField] } })
                const targetTableFields = await this.#__fields.findAll({ table_id: targetTable.id })
                if (fields.length == 2) {
                    ...
                    if (index && field) {
                        ...
                        const currentValues = await Database.selectAll("RFID_AMOUNT_CODE_MARK")
                        let currentValue = 63_710_065_410_048;
                        if (currentValues && currentValues.length > 0) {
                            const record = currentValues[0]
                            currentValue = record ? record.value : 0
                            await Database.update("RFID_AMOUNT_CODE_MARK", {}, { value: currentValue + amount })
                        } else {
                            await Database.insert("RFID_AMOUNT_CODE_MARK", { value: amount + currentValue })
                        }


                        const { foreign_keys } = targetTable;
                        const targetKey = foreign_keys.find(key => key.table_id == foreign_table)
                        if (targetKey) {
                            const { field_id, table_id } = targetKey;
                            const foreignField = targetTableFields.find(f => f.id == field_id)
                            const primaryTables = await this.#__tables.findAll({ id: table_id })
                            const primaryTable = primaryTables[0]

                            const { primary_key } = primaryTable
                            const primalField = await this.#__fields.find({ id: primary_key[0] })

                            const dataset = []

                            const primaryData = await Database.selectAll( primaryTable.table_alias, { [primalField.fomular_alias]: foreign_value })
                            if( primaryData.length > 0 ){
                                ...
                                let cache = await Cache.getData(`${targetTable.table_alias}-periods`)
                                if (!cache) {
                                    cache = {
                                ...
                                const sumerizes = await Database.selectAll(targetTable.table_alias, { position: "sumerize" })
                                ...
                                await Cache.setData(`${targetTable.table_alias}-periods`, periods)
    
                                sumerize.total += dataset.length
                                await Database.update(targetTable.table_alias, { position: "sumerize" }, { ...sumerize })
